...
def run_sims (apsim, sims_path="apsim_files/Greene/*.apsim"):
    ...
    Arguments:
        ...
        sims_path {str} -- Path to .apsim files
    """
    wd = getcwd()
    sims = glob(sims_path)
    complete_file_paths = list()
    total_sims = len(sims)
    print(f'Running APSIM for {total_sims} files.')
    sims_count = 0
    failed_sims = 0
    for i in sims:
    ...
    for i in complete_file_paths:
        #run exe on each .apsim file
            ...
            if run != 0:
                failed_sims += 1
                print(f'!!!-------Something went wrong with sim {i}--------!!!')
            sims_count += 1
            if sims_count % 10 == 0:
                print(f"----Executing simulation {sims_count} of {total_sims}----")
    print(f'{failed_sims} sims failed out of {total_sims}')
